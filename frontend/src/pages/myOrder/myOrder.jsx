  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3000/orders/client/orders', {
          headers: {
              ...
              <div className="order-summary" onClick={() => toggleOrderDetails(order._id)}>
                <div className="order-main-info">
                  <div className="client-info">
                    <img 
                      src={order.professional?.image || "https://avatar.iran.liara.run/public/boy?username=User"} 
                      alt={(order.professional?.fname || 'Client')} 
                      className="client-avatar" 
                    />
                    <div>
                      <h3>{order.professional?.fname || 'Client'} {order.professional?.lname || ''}</h3>
                ...
                <div className="order-dates">
                  ...
                  <span>Deliverable Date: {formatDate(order.deadline)}</span>
                </div>
              </div>
            </div>
          ))}
